...
void loop() {
  ...
  if (centerValue > 5) { 
    int s1 = speed1+((correction*3)/2); // left motor
    int s2 = speed2-((correction*1)/2); // right motor
  } else if (correction > 0) { // right turn
    int s1 = speed1+((correction*4)/2); // left motor gets more power to turn right
    int s2 = speed2-((correction*1)/2); // right motor
  } else if (correction < 0) { // left turn
    int s1 = speed1+((correction*3)/2); // left motor
    int s2 = speed2-(correction); // right motor gets more power to turn left
  }
